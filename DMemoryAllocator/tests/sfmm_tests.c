...
//Test #1: Memalign Test 
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {

	void *a = sf_memalign(1000, 16);

	cr_assert(((size_t)a % 16) == 0, "The pointer is not aligned");
...
Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT) {
	...
	void *b = sf_memalign(45, 128);

	cr_assert(((size_t)b % 128) == 0, "The pointer is not aligned");
...
Test(sfmm_student_suite, student_test_3, .timeout = TEST_TIMEOUT){
	...
	void *c = sf_memalign(1020, 512);

	cr_assert(((size_t)c % 512) == 0, "The pointer is not aligned");
...
Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT) {
	...
	void *d = sf_memalign(8000, 32);

	cr_assert(((size_t)d % 32) == 0, "The pointer is not aligned");
...
Test(sfmm_student_suite, student_test_5, .timeout = TEST_TIMEOUT) {
	...
	void *e = sf_memalign(3456, 64);

	cr_assert(((size_t)e % 64) == 0, "The pointer is not aligned");
